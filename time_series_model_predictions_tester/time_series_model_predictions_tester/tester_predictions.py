...
def generate_queries(client_list):
    ...
    for client in client_list:
        if client in ('PIG930806GL1', 'CJF941024F7A', 'OEB1410238K9', 'CDJ2011111U4', 'ANL171011549'):
            ...
            query = f"{base_query}'{client}') AND t.tran_fechahora >= NOW() - INTERVAL 3 DAY;"
...
def df_creator_hourly(files_names):
    ...
    for file_name in files_names:
        ...
        if not file_df.empty:
            ...
            test_file_data_2hourly.to_csv(f"{path_to_save}_2hourly_" + file_name, index=False)
            ...
            df_blanco.to_csv(f"{path_to_save}_2hourly_" + file_name, index=False)
...
data_hist = pd.read_csv("bandas_historic.csv", index_col = 'txn_date')
...
file_name = "anomalies_refresh.csv"
